...
local theme = lush(function()
  return {
    ...
    Visual { bg = CursorLine.bg, fg = Normal.fg.rotate(100) }, -- Try pressing v and selecting some text

    -- We can adjust our comments to look like desaturate normal text
    Comment { fg = Normal.bg.de(25).li(25).ro(-10), gui = 'italic' },
    ...
    -- Syntax groups (treesitter @captures fall back to these)
    Constant       { fg = sea_foam.ro(30).sa(20) },
    String         { fg = sea_foam.ro(80).de(10) },
    Character      { String },
    Number         { fg = sea_foam.ro(-40).sa(30) },
    Boolean        { Number, gui = "bold" },
    Float          { Number },

    Identifier     { fg = sea_foam },
    Function       { fg = sea_foam.ro(180).li(20).sa(40) },

    Statement      { fg = sea_foam.ro(-60).sa(40), gui = "bold" },
    Conditional    { Statement },
    Repeat         { Statement },
    Label          { Statement },
    Operator       { fg = sea_gull },
    Keyword        { Statement },
    Exception      { Statement },

    PreProc        { fg = sea_foam_triadic },
    Include        { PreProc },
    Define         { PreProc },
    Macro          { PreProc },
    PreCondit      { PreProc },

    Type           { fg = sea_foam_complement.li(30) },
    StorageClass   { Type, gui = "italic" },
    Structure      { Type },
    Typedef        { Type },

    Special        { fg = sea_foam.ro(60).sa(30) },
    SpecialChar    { Special },
    Tag            { Special },
    Delimiter      { fg = sea_gull.da(20) },
    SpecialComment { Comment, gui = "bold" },
    Debug          { Special },

    Underlined     { gui = "underline" },
    Error          { fg = hsl(0, 80, 60) },
    Todo           { fg = hsl(50, 90, 60), gui = "bold" },

    -- Treesitter capture overrides for groups that don't auto-link cleanly
    sym"@variable"        { Identifier },
    sym"@variable.member" { Identifier.fg.li(10) },
    sym"@property"        { Identifier.fg.li(10) },
    sym"@parameter"       { Identifier, gui = "italic" },
    sym"@field"           { Identifier.fg.li(10) },
